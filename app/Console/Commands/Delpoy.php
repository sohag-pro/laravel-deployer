...
class Delpoy extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            // Dump DB to db dir
            echo exec("mysqldump -u $db_user -p$db_password -h $db_host -P $db_port $db_name > $db_dir/$db_name-$tag.sql");

            // link .env to version/.env
            echo exec("ln -s $base_dir/.env $version_dir/.env");

            // run composer install in version directory
            echo exec("cd $version_dir && $commands");
